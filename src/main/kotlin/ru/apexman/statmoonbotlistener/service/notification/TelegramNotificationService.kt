...
import ru.apexman.statmoonbotlistener.dto.TelegramDocumentDto
...
class TelegramNotificationService(
    ...
    fun trySendMessage(text: String) {
        for (chatId in telegramConfiguration.chatIds) {
            try {
                sendMessage(chatId, text)
            } catch (e: Exception) {
                val errorMessage = "Failed to send to chat $chatId, text: $text"
    ...
    private fun sendMessage(
        ...
        if (!telegramConfiguration.enabled) {
    ...
    companion object {
        fun buildTelegramDocumentDto(exception: Exception): TelegramDocumentDto {
            val stringWriter = StringWriter()
            val printWriter = PrintWriter(stringWriter)
            exception.printStackTrace(printWriter)
            printWriter.close()
            return TelegramDocumentDto(name = "exception.log", content = stringWriter.toString())
        }
    }
